...
## Step 2 - Care base (always) - MONTHLY-OPS-GUIDE §0
Uptime/TLS, dependency+security (engine `OPERATIONS.md` §4.1), backup check, IndexNow re-index of changed
...
- **SEO/AEO Ops** (§1) -> `claude-seo` audit + GSC page-2 internal links + language mining + structured-data
  check + IndexNow.
- **Content** (§2) -> AnswerThePublic/GSC questions -> interview -> `content-ops` (voice-dna + anti-ai-writing)
...
- **GBP Management** (§3) -> BrightLocal citations + services optimization + review replies + GBP posts +
  local rank tracking.
- **Reviews** (§4) -> confirm the engine review-request flow is firing / send the monthly ask; moderate +
  respond.
- **Backlinks** (§5) -> EARNED only (competitor analysis, link-worthy asset, real pitches). Never buy/exchange/
  automate links.

## Step 4 - AI-visibility check - §6
Run 3-5 buyer prompts in ChatGPT/Perplexity/Gemini/Claude; record mentions vs competitors; note one action.

## Step 5 - Draft the monthly report - §7
